            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 border border-gray-700 rounded-lg bg-gray-800 text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"
                  ...
                  <button
                    ...
                    className="shrink-0 px-3 py-2 text-sm bg-gray-800 hover:bg-gray-700 disabled:opacity-50 text-gray-300 rounded-lg border border-gray-700 transition"
                  >
                    {detecting ? "検出中..." : "RSS検出"}
                  </button>
                </div>
                {detectMsg && (
                  <p className={`text-xs mt-1 ${detectMsg.includes("検出しました") ? "text-green-400" : "text-red-400"}`}>
                    {detectMsg}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-gray-700 rounded-lg bg-gray-800 text-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="button"
                  onClick={closeForm}
                  className="flex-1 py-2.5 border border-gray-700 text-gray-300 hover:bg-gray-800 font-medium rounded-lg transition"
                >
                  キャンセル
                </button>
              </div>
      ...
      {sites.length === 0 ? (
        ...
        <div className="space-y-2">
          {sites.map((site) => (
            <div
              key={site.id}
              className={`flex items-center gap-3 p-4 bg-gray-900 rounded-xl border ${
                site.isActive ? "border-gray-800" : "border-gray-900 opacity-60"
              }`}
            >
              <div className={`w-2 h-2 rounded-full shrink-0 ${site.isActive ? "bg-green-500" : "bg-gray-400"}`} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-gray-100 truncate text-sm">{site.name}</span>
                  {site.category && (
                    <span className="text-xs bg-gray-800 text-gray-400 px-2 py-0.5 rounded-full shrink-0">{site.category}</span>
                  )}
                </div>
                <p className="text-xs text-gray-500 truncate mt-0.5">{site.feedUrl}</p>
                {site.lastFetched && (
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="text-xs px-2.5 py-1 rounded-lg border border-gray-700 text-gray-400 hover:bg-gray-800 transition"
                >
                  編集
                </button>
                <button
                  onClick={() => handleDelete(site.id)}
                  className="text-xs px-2.5 py-1 rounded-lg border border-red-900 text-red-400 hover:bg-red-950 transition"
                >
                  削除
                </button>
              </div>
